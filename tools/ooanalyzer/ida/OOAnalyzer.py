...
class OOAnalyzerJsonStrings:
    '''
    A set of properties for JSON types
    '''

    @property
    def Types(self): return "types"

    @property
    def Count(self): return "count"

    @property
    def Usages(self): return "Usages"
    ...
    @property
    def Dtor(self): return "dtor"

    @property
    def Ctor(self): return "Ctor"

    @property
    def Meth(self): return "meth"

    @property
    def Entries(self): return "entries"

    @property
    def Struc(self): return "struc"
...
class PyOOAnalyzer(object):
    ...
    def parse(self):
        ...
        if "Usages" in data:
            self.__parse_usages(data[OOAJson.Usages])
        ...
        for s in structs:
            ...
            for c in self.__classes:
                if c.name == s['Name']:
                    ...
                    if OOAJson.Vftables in s: self.__parse_vftables(c, s[OOAJson.Vftables])
                    if OOAJson.Members in s: self.__parse_members(c, s[OOAJson.Members])
                    if OOAJson.Methods in s: self.__parse_methods(c, s[OOAJson.Methods])
    ...
    def __parse_members(self, cls, members):
        ...
        for m in members:
            ...
            offset = int(m[OOAJson.Offset], 16)
            ...
            mem = PyClassMember()
            mem.cls = cls
            mem.member_type = m[OOAJson.Type]
            mem.count = int(m[OOAJson.Count])

            if mem.member_type == FF_STRU:

                class_member_name = m[OOAJson.Struc]  # This could be mangled
                cls_mem = None
                for c in self.__classes:
    ...
    def __parse_methods(self, cls, methods):
        ...
        for m in methods:
            ...
            meth.is_ctor = False
            meth.is_dtor = False
            if m[OOAJson.Type] == "ctor":
                meth.is_ctor = True
            elif m[OOAJson.Type] == "dtor":
                meth.is_dtor = True

            #  this method is imported
            meth.is_import = False
            if m[OOAJson.Imported] == "yes":  meth.is_import = True

            meth.start_ea = int(m[OOAJson.EA], 16)

            meth.method_name = m[OOAJson.Name]
    ...
    def __parse_vftables(self, cls, vftables):
        ...
        for v in vftables:
            ...
            vft = PyClassVftable()
            vft.cls = cls
            vft.start_ea = int(v[OOAJson.EA], 16)

            if OOAJson.Entries not in v: continue

            for entry in v[OOAJson.Entries]:
                meth = PyClassMethod()

                meth.cls = cls
                meth.start_ea = int(entry[OOAJson.EA], 16)
                # meth.method_name = "%s_%x" % (entry['name'], meth.start_ea)
                meth.method_name = entry[OOAJson.Name]

                meth.is_virtual = True

                meth.is_ctor = False
                meth.is_dtor = False
                if entry[OOAJson.Type] == "ctor":
                    meth.is_ctor = True
                elif entry[OOAJson.Type] == "dtor":
                    meth.is_dtor = True
                ...
                vft.add_virtual_function(meth, int(entry[OOAJson.Offset]))
